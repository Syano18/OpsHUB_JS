...
function getEffectiveAnnualAllocation(leaveType, annualAllocation) {
  ...
  if (normalizedLeaveType === 'use_leave') {
    return Math.max(6, normalizedAnnualAllocation);
  }
...
async function getAvailableLeaveBalanceForDates({
  ...
  const [userRes, balancesResult, pendingRequestsResult] = await Promise.all([
    tursoClient.execute({
      sql: `SELECT emp_stat FROM User_Permissions WHERE lower(Email) = ? LIMIT 1`,
  ...
  const empStat = String(userRes.rows[0]?.emp_stat || '').trim().toUpperCase();

  if (empStat === 'COSW' && normalizedLeaveType !== 'wellness') {
    return 0;
  }
  ...
  const effectiveAnnualAllocation = getEffectiveAnnualAllocation(
    normalizedLeaveType,
...
export async function ensureLeaveBalanceRecord(email, fullName, leaveType, year, allocationValue) {
  const defaultAlloc = Number(allocationValue ?? 0);
  const existing = await tursoClient.execute({
    sql: `SELECT id FROM leave_balances WHERE lower(employee_email) = ? AND leave_type = ? AND balance_year = ? LIMIT 1`,
    args: [email, leaveType, year],
  });
  ...
  if (existing.rows.length === 0) {
    await tursoClient.execute({
      sql: `
        ...
        (employee_email, employee_name, leave_type, balance_year, earned, used, restored, adjusted, balance)
        VALUES (?, ?, ?, ?, ?, 0, 0, 0, ?)
      `,
      args: [email, fullName, leaveType, year, defaultAlloc, defaultAlloc],
    });
  }
}
...
export async function getLeaveMonitoringData(employeeEmail, balanceYear) {
  ...
  const [userPermissionResult, leaveTypesResult, balancesResult, requestsResult, pendingRequestsResult] = await Promise.all([
    tursoClient.execute({
      sql: `
        SELECT emp_stat
  ...
  const empStat = String(userPermissionResult.rows[0]?.emp_stat || '').trim().toUpperCase();
  ...
  const pendingDaysByType = new Map();
  ...
  for (const row of pendingRequestsResult.rows) {
    ...
    const currentPending = pendingDaysByType.get(leaveType) ?? {
      total: 0,
      byYear: new Map(),
    };
    const pendingDays = normalizeNumeric(row.requested_days || 1);
    const pendingYear = Number.parseInt(String(row.date_from ?? '').slice(0, 4), 10);

    currentPending.total += pendingDays;

    if (!Number.isNaN(pendingYear)) {
      currentPending.byYear.set(
        pendingYear,
        normalizeNumeric(currentPending.byYear.get(pendingYear)) + pendingDays
      );
    }

    pendingDaysByType.set(leaveType, currentPending);
  }

  return leaveTypesResult.rows
    .filter((row) => {
      if (empStat === 'COSW') {
        return row.code === 'wellness';
      }
    ...
    .map((row) => {
      ...
      const pendingBalance = pendingDaysByType.get(leaveType) ?? {
        total: 0,
        byYear: new Map(),
      };
      ...
      if (resetPolicy === 'yearly' && !relevantBalances.length) {
        totals.balance = effectiveAnnualAllocation;
      }

      const pendingDays =
        resetPolicy === 'lifetime'
          ? normalizeNumeric(pendingBalance.total)
          : normalizeNumeric(pendingBalance.byYear.get(normalizedBalanceYear));
      const availableBalance = Math.max(0, totals.balance - pendingDays);
...
export async function createLeaveRequest({
  ...
  if (String(userRes.rows[0]?.emp_stat || '').trim().toUpperCase() === 'COSW' && normalizedLeaveType !== 'wellness') {
    throw new Error('COSW employees are only eligible for Wellness Leave.');
  }
...
export async function updateLeaveRequestGroupStatus({
  ...
  if (String(userRes.rows[0]?.emp_stat || '').trim().toUpperCase() === 'COSW' && leaveRequestGroup.leaveType !== 'wellness' && normalizedStatus === 'approved') {
    throw new Error('COSW employees are only eligible for Wellness Leave.');
  }
  ...
  if (normalizedStatus === 'approved') {
    for (const row of leaveRequestGroup.rows) {
      ...
      // Sync approved leave to Personal Events calendar
      const leaveLabel =
        leaveTypeSettings.name ||
        leaveRequestGroup.leaveType
          .split('_')
