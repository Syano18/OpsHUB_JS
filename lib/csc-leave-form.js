...
import {
  ...
  CSC_FORM_MOCK_GROUP_ID,
  getCscForm6ViewData,
  getMockCscForm6ViewData,
...
function createPdfLayout(template, bounds) {
  ...
  return {
    minX: bounds.minX,
    minY: bounds.minY,
    padX: Math.max(0, (template.pageWidth - contentWidth) / 2) + 10,
    padY: Math.max(0, (template.pageHeight - contentHeight) / 2),
    scale: 1,
...
function scaleX(x, layout) {
  return layout.padX + (x - layout.minX) * layout.scale;
}

function scaleTopY(y, layout) {
  return layout.padY + (y - layout.minY) * layout.scale;
}
...
function drawTemplateText(page, template, fonts, layout) {
  for (const line of template.lines) {
    const baseFontSize = line.fixedFontSize ?? Math.max(8.5, line.height * 0.92);
    const y =
      layout.pageHeight -
      scaleTopY(line.topY1, layout) +
      0.5 -
      (line.yOffset || 0) * layout.scale;

    if (line.parts?.length) {
      const partPadding = line.padding ?? getTextBoxPadding(baseFontSize);
      const partMaxWidth = Math.max(1, line.width - partPadding * 2);
    ...
    const textPadding = line.padding ?? (line.disableAutoSize ? 0 : getTextBoxPadding(baseFontSize));
    const maxTextWidth = Math.max(1, line.width - textPadding * 2);
...
function drawTemplateRectangleBorders(page, template, layout) {
  ...
  for (const drawing of template.drawings) {
    ...
    const width = drawing.width * layout.scale;
    const height = drawing.height * layout.scale;
    const yBottom = yTop - height;

    if (isCheckboxDrawing(drawing)) {
      const checkboxBorderWidth = Math.max(0.8, (drawing.strokeWidth || 0.8) * layout.scale);
      page.drawRectangle({
        x,
        y: yBottom,
        width,
    ...
    const thickness = Math.max(0.9, borderWidth * layout.scale);
    const common = {
      thickness,
      color: lineColor,
...
async function drawTemplateImages(pdf, page, template, layout) {
  ...
  for (const image of embeddedImages) {
    const boxWidth = image.width * layout.scale;
    const boxHeight = image.height * layout.scale;
...
function drawDynamicEntries(page, template, entries, fonts, layout) {
  ...
  for (const entry of entries.values) {
    ...
    const baseSize = entry.size || 8;
    const entryPadding = entry.padding ?? getTextBoxPadding(baseSize);
    const entryMaxWidth = Math.max(1, entry.width - entryPadding * 2);
    ...
    lines.forEach((line, index) => {
      ...
      const y =
        ...
        size +
        1 -
        index * (entry.lineHeight || baseSize * 1.2);
...
export async function generateCscForm6Pdf(groupIdentifier, options = {}) {
  ...
  const isMockGroup =
    ...
    normalizeText(groupIdentifier) === CSC_FORM_MOCK_GROUP_ID;

  const formDataPromise = isMockGroup
    ? Promise.resolve(getMockCscForm6ViewData())
    : prefetchedLeaveRequestGroup && prefetchedEmployee
