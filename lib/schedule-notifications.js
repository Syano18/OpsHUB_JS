...
const DEFAULT_NOTIFICATION_CONCURRENCY = 4;
...
function getNotificationConcurrency(recipientCount) {
  const concurrency = normalizePositiveInteger(
    process.env.SCHEDULE_NOTIFICATION_CONCURRENCY,
    DEFAULT_NOTIFICATION_CONCURRENCY
  );

  return Math.max(1, Math.min(concurrency, recipientCount));
}
...
export async function sendScheduleAssignmentNotifications({
  ...
  const recipients = normalizeRecipients(
    await resolveRecipientDirectory(schedule?.assignedTo)
  );
  ...
  const workerCount = getNotificationConcurrency(recipients.length);
    ...
    Array.from({ length: workerCount }, async () => {
      while (true) {
        ...
        try {
          ...
          results[recipientIndex] = { status: 'fulfilled', value: result };
        } catch (error) {
          results[recipientIndex] = { status: 'rejected', reason: error };
        }
